# блок запроса данных от источника и перегрузки их с учетом повторов в бд
...
def load():
    ...
    for x in citys.geo.keys():
        ...
        for data in responce['list']:
            ...
            # проверка дублей
            if db_sess.query(Weather).filter(Weather.date == dt.datetime.strptime(data['dt_txt'], '%Y-%m-%d %H:%M:%S'),
                                             Weather.city == x):
                # существующую старую запись удаляем
                db_sess.query(Weather).filter(Weather.date == dt.datetime.strptime(data['dt_txt'], '%Y-%m-%d %H:%M:%S'),
